...
class ICM42688
{
  public:
	 ...
	 */
	int8_t begin_SPI(uint8_t cs_pin, SPIClass *theSPI = &SPI, uint32_t frequency = 8000000);
	 ...
	 *    @return True if initialization was successful, otherwise false.
	 */
	int8_t begin_SPI(int8_t cs_pin, int8_t sck_pin, int8_t miso_pin,
                 int8_t mosi_pin, uint32_t frequency = 8000000);
	
	/**
     *@brief	   Initialize the device with I2C.
	 *
     * @param  i2c_addr  Address of ICM-42688 device
	 * @param[in]  *wire  TwoWire instance to use
	 * @return     ret < 0 if error
     */
	int8_t begin_I2C(uint8_t i2c_addr = 0x68,
						TwoWire *wire = &Wire);
  ...
  protected:
    ...
    ///\brief Constants
	static constexpr uint8_t WHO_AM_I_P = 0x47; ///< expected value in UB0_REG_WHO_AM_I reg of P variant
    static constexpr uint8_t WHO_AM_I_V = 0xDB; ///< expected value in UB0_REG_WHO_AM_I reg of V variant
