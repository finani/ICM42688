// part of the code taken from Adafruit BusIO, LSM6DS
...
// initialize ICM42688
int8_t ICM42688::init(void) {
    // reset the ICM42688
  reset();

  // check the WHO AM I byte with two chip variant
  if((whoAmI() != WHO_AM_I_P)||(whoAmI() != WHO_AM_I_V)) {
    return -3;
  }
...
int8_t ICM42688::begin_SPI(uint8_t cs_pin, SPIClass *theSPI,
						   uint32_t frequency) {
	_spi = theSPI; // SPI bus
...
int8_t ICM42688::begin_SPI(int8_t cs_pin, int8_t sck_pin, int8_t miso_pin,
...
int8_t ICM42688::begin_I2C(uint8_t address, TwoWire *wire)
{
	_useSPI = false;
